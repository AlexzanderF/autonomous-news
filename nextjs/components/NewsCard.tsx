  ...
  return (
    <Link href={`/article/${item.slug}`}>
      <article className="group relative bg-white border border-slate-200 hover:border-indigo-500/50 transition-all duration-300 overflow-hidden hover:shadow-lg hover:shadow-indigo-500/10 cursor-pointer h-full rounded-lg flex flex-col">
        {/* Image */}
        <div className="aspect-video w-full overflow-hidden relative z-0 flex-shrink-0 bg-slate-100">
          <SmartImage
            src={item.imageUrl}
            alt={item.headline}
            fill
        ...
        <div className="relative z-20 p-5 flex flex-col flex-grow">
          ...
          <h3 className="font-bold leading-tight mb-2 text-slate-900 group-hover:text-indigo-700 transition-colors font-sans text-lg">
            {item.headline}
          </h3>

          <p className="text-slate-700 mb-2 font-light leading-relaxed text-sm line-clamp-3">
            {item.summary}
          </p>
